...
axiosInstance.interceptors.request.use(
    ...
    (error) => {
        const errorToThrow = error instanceof Error ? error : new Error('Request error');
...
axiosInstance.interceptors.response.use(
    ...
    async (error) => {
        ...
        const errorToReject = error instanceof Error ? error : new Error('Response error');
...
class ApiService {
    ...
    private handleError(error: unknown): Error {
        if (error && typeof error === 'object' && 'response' in error) {          
            ...
            const message = axiosError.response.data?.message || axiosError.response.data?.detail || 'Error del servidor';
